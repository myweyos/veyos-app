...
Golden fixtures F14, F17 and F18 cover the same thing on the product rulebook and real personas.
"""
...
BOOK = load_rulebook()
PERSONAS = Path(__file__).resolve().parents[3] / "packages" / "demo-fixtures" / "personas.json"
...
def test_unknown_is_traced_and_false_is_not() -> None:
    ...
    James on a calm day: HRV present and near baseline, so 1.1 is FALSE and leaves no row.
    Knock HRV out and 1.1 is UNKNOWN, so it gets an "unevaluable" row and still doesn't fire.
    """
    raw = json.loads(PERSONAS.read_text(encoding="utf-8"))["james"]["calm"]
