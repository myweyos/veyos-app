...
|---|---|---|---|---|---|
| **R1** | **False reassurance: the RHR-alone gap.** RHR +26% above baseline with a temperature that is present and normal → rule 1.3's dual gate is FALSE → state `calm`, "in balance today". (With temperature *missing*, 1.3 is unevaluable and the day is Partial; see R2) | User trains hard or ignores a developing illness | S3 | C3 (F19 pins it, F5b xfail), signal tiles always visible, so the RHR reading is on screen | **Open.** Candidate rule 1.4 unapproved (plan v2 §9.3). The fixture fix §3 asked for first is done (F5 vs F19, PR #19) |
...
| **R20** | **Fixture data reaches a real user** | Advice for someone else's body | S3 | CLAUDE.md non-negotiable 9 | **Closed by PR #19.** The API's fixture path (`personas.source.ts`, `WEYOS_DEMO_FIXTURES`) and every persona are deleted; all data routes serve the signed-in subject's stored data only |
...
1. **R20:** done in PR #19. The fixture path is removed entirely, not just defaulted off.
