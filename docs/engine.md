...
|---|---|
| F5 | James as the design pack has him: RHR 26% up, **no** wrist temperature. 1.3 is unevaluable, not false, so the day is Partial rather than "in balance". |
| **F19** | **The James gap.** RHR 26% above baseline with a temperature that is present and normal fires nothing but L3, so the app says "in balance today". Known product defect, awaiting a rule decision. (It was F5 until the persona was aligned with the pack.) |
